...
def test_create_student(client):
    ...
    product_id = create_dependencies(client, token)
    headers = {'Authorization': f'Bearer {token}'}
    response = client.post('/api/student/create',
        data=json.dumps({
            'name': 'João Silva',
...
def test_list_students(client):
    token, _ = get_token(client)
    headers = {'Authorization': f'Bearer {token}'}
    response = client.get('/api/student/read/all', headers=headers)
    print(response.status_code)
    print(response.get_json())
...
def test_read_student_by_id(client):
    token, _ = get_token(client)
    headers = {'Authorization': f'Bearer {token}'}
    product_id = create_dependencies(client, token)
    resp = client.post('/api/student/create',
        data=json.dumps({'name': 'João Silva', 'product_id': product_id, 'room_id': 1, 'admin_id': 1}),
        content_type='application/json', headers=headers)
    student_id = resp.get_json()['data']['id']
    response = client.get(f'/api/student/read/{student_id}', headers=headers)
    print(response.status_code)
    print(response.get_json())
    assert response.status_code == 200
...
def test_update_student(client):
    ...
    resp = client.post('/api/student/create',
        ...
        content_type='application/json', headers=headers)
    student_id = resp.get_json()['data']['id']
    response = client.put(f'/api/student/update/{student_id}',
        data=json.dumps({'name': 'Maria Souza', 'product_id': product_id, 'room_id': 1, 'admin_id': 1}),
        content_type='application/json', headers=headers)
    print(response.status_code)
    print(response.get_json())
    assert response.status_code == 200
...
def test_delete_student(client):
    ...
    resp = client.post('/api/student/create',
        ...
        content_type='application/json', headers=headers)
    student_id = resp.get_json()['data']['id']
    response = client.delete(f'/api/student/delete/{student_id}', headers=headers)
    print(response.status_code)
    print(response.get_json())
    assert response.status_code == 200
    response = client.get(f'/api/student/read/{student_id}', headers=headers)
